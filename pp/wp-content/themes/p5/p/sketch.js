...
var spec = {
  ...
  onDraw: function() {
    ...
    // ====================== first loop
    {
      ...
      beginShape();
      vertex(this.x_min, this.y_max);
      for (i = 0; i < spectrum.length; i++) {
        var eng = spectrum[i];


        y = this.engToY(eng);
        x = i * this.x_scale + this.x_min;
        if (x > this.x_max) break;
        vertex(x, y);

      }
      vertex(this.x_max, this.y_max);
      endShape();
    }

    if (data.debug) {
      fill('#f00');
      for (i = 0; i < data.tops.length; i++) {
        var top = data.tops[i];
        if (!top) continue;
        if (i > data.x_max) break;
        x = top.index * this.x_scale + this.x_min;
        y = this.engToY(spectrum[i]);
        ellipse(x, y, 4);
      }

      noFill();
      stroke(data.spec_color_stroke);
      beginShape();
      vertex(this.x_min, this.y_max);
      for (i = 0; i < spectrum.length; i++) {
        var eng = spectrum[i];
        y = map(eng, 0, data.max_eng, this.y_max, this.y_min);
        x = i * this.x_scale + this.x_min;
        if (x > this.x_max) break;
        vertex(x, y);
      }
    ...
    noFill();
    if (data.show_pitch_on_spec && data.pitch && data.max_top) {
      stroke(data.pitch_color);
  ...
  xToFreq: function(x) {
    x -= this.x_min;
    return map(x / this.x_scale, 0, data.fft_size, 0, data.sample_rate / 2);
  },
  freqToX: function(freq) {
    var x = map(freq, 0, data.sample_rate / 2, 0, data.fft_size);
    return this.x_min + x * this.x_scale;
  },
...
var piano = {
    ...
    for (var x = 0; x < this.width; ++x) {
      for (var y = 0; y < this.height; y++) {
        if (x === 0 || y === 0 || x == this.width - 1 || y == this.height - 1) {
          ...
          var pitch = this.pointToPitch(x, y);
          this.buffer.fillPoint(x, y, pitch.is_white ? white_color : black_color);
        }

      }
    }
    for (var i = 1; i < PITCH_DATA.length; ++i) {
      var pitch = PITCH_DATA[i];
      var x = this.getLeft(pitch) - this.x_min;
      x = Math.round(x);
      if (pitch.is_white) {
        for (var y = 0; y < this.height; ++y) {
          this.buffer.fillPoint(x, y, black_color);
  ...
  init: function(x1, x2, y1, y2) {
    ...
    this.whites = new Array(PITCH_DATA.length);
    this.blacks = new Array(PITCH_DATA.length);
    var index = 0;
    for (var i = 0; i < PITCH_DATA.length; ++i) {
      var pitch = PITCH_DATA[i];
      pitch.piano_index = index;
      if (pitch.inter.length == 2) {
        pitch.is_white = true;
        this.whites[index] = pitch;
        index++;
      } else {
        pitch.piano_index = index;
        pitch.is_white = false;
        this.blacks[index] = pitch;
      }
  ...
  pointToPitch: function(x, y) {
    var x_pos = x / this.white_width;
    var index = Math.floor(x_pos);
    if (y < this.black_height) {
      var x_detail_pos = x_pos - index;
      if (this.blacks[index] && x_detail_pos < this.black_x_ratio / 2) {
        return this.blacks[index];
      }
      if (this.blacks[index + 1] && x_detail_pos > 1 - this.black_x_ratio / 2) {
        return this.blacks[index + 1];
      }
    }

    return this.whites[index];
